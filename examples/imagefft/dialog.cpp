...
void calcFFT(const cv::Mat &input, cv::Mat &output) {
  ...
  cv::copyMakeBorder(input, padded, 0, m - input.rows, 0, n - input.cols, cv::BORDER_CONSTANT,
                     cv::Scalar_<float>::all(0));
  ...
  cv::normalize(output, output, 0, 1, cv::NORM_MINMAX);        // Transform the matrix with float values into a
