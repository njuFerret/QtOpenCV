...
void process1() {
  ...
  // Canny detector
  cv::Canny(detected_edges, detected_edges, 20, 60, kernel_size);
  ...
  cv::Mat elements =
      cv::getStructuringElement(2, cv::Size(2 * dilation_size2 + 1, 2 * dilation_size2 + 1),
  ...
  cv::findContours(mat_dilation2, contours, find_contours_mode, cv::CHAIN_APPROX_NONE);
  ...
  for (size_t i = 0, foundCount = 0; (i < contours.size()) && (foundCount < maxEllipseCount); i++) {
    ...
    cv::ellipse(ellipseImage, box, cv::Scalar(0, 0, 255), 1, cv::LINE_AA);
    cv::ellipse(ellipseImage, box.center, box.size * 0.5f, box.angle, 0, 360, cv::Scalar(0, 255, 255), 1, cv::LINE_AA);
    ...
    if (foundCount == 2) {
      ...
      box.points(vtx);
      for (int j = 0; j < 4; j++)
        cv::line(ellipseImage, vtx[j], vtx[(j + 1) % 4], cv::Scalar(0, 255, 0), 1, cv::LINE_AA);
    }
