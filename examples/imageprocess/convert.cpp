...
QString AbstractConvert::errorString() const { return m_errorString; }
...
bool BilateralFilter::applyTo(const cv::Mat &input, cv::Mat &output) {
  cv::bilateralFilter(input, output, dEdit->value(), sigmaColorEdit->value(), sigmaSpaceEdit->value(),
                      borderTypeEdit->itemData(borderTypeEdit->currentIndex()).toInt());
  return true;
}
...
bool BoxFilter::applyTo(const cv::Mat &input, cv::Mat &output) {
  cv::boxFilter(input, output, -1, cv::Size(kSizeXEdit->value(), kSizeYEdit->value()),
                cv::Point(anchorXEdit->value(), anchorYEdit->value()),
                normalizeEdit->itemData(borderTypeEdit->currentIndex()).toBool(),
                borderTypeEdit->itemData(borderTypeEdit->currentIndex()).toInt());
  return true;
}
...
bool GaussianBlur::applyTo(const cv::Mat &input, cv::Mat &output) {
  cv::GaussianBlur(input, output, cv::Size(kSizeXEdit->value(), kSizeYEdit->value()), sigmaXEdit->value(),
                   sigmaYEdit->value(), borderTypeEdit->itemData(borderTypeEdit->currentIndex()).toInt());
  return true;
}
...
bool Canny::applyTo(const cv::Mat &input, cv::Mat &output) {
  cv::Canny(input, output, threshold1Edit->value(), threshold2Edit->value(), apertureSizeEdit->value(),
            l2gradientButton->isChecked());
  return true;
}
...
bool Dilate::applyTo(const cv::Mat &input, cv::Mat &output) {
  cv::Mat kernel = cv::getStructuringElement(kShapeEdit->itemData(kShapeEdit->currentIndex()).toInt(),
                                             ...
                                             cv::Point(anchorXEdit->value(), anchorYEdit->value()));
  cv::dilate(input, output, kernel, cv::Point(anchorXEdit->value(), anchorYEdit->value()), iterationsEdit->value(),
             borderTypeEdit->itemData(borderTypeEdit->currentIndex()).toInt());
  return true;
}
...
bool Erode::applyTo(const cv::Mat &input, cv::Mat &output) {
  cv::Mat kernel = cv::getStructuringElement(kShapeEdit->itemData(kShapeEdit->currentIndex()).toInt(),
                                             cv::Size(kSizeXEdit->value(), kSizeYEdit->value()),
                                             cv::Point(anchorXEdit->value(), anchorYEdit->value()));
  cv::erode(input, output, kernel, cv::Point(anchorXEdit->value(), anchorYEdit->value()), iterationsEdit->value(),
            borderTypeEdit->itemData(borderTypeEdit->currentIndex()).toInt());
  return true;
}

bool HoughCircles::applyTo(const cv::Mat &input, cv::Mat &output) {
  qDebug() << input.dims << input.channels() << input.rows << input.cols;
  qDebug() << input.type() << input.empty() << CV_8UC1 << CV_8UC3;

  std::vector<cv::Vec3f> circles;
  // cv::HoughCircles(input, circles, cv::HOUGH_GRADIENT, dpEdit->value(), minDistEdit->value(), param1Edit->value(),
  //                  param2Edit->value(), minRadiusEdit->value(), maxRadiusEdit->value());
  cv::Mat tmp;
  if (input.type() == CV_8UC3) {
    cv::cvtColor(input, tmp, cv::COLOR_BGR2GRAY);
  }
  cv::HoughCircles(tmp, circles, cv::HOUGH_GRADIENT, dpEdit->value(), minDistEdit->value(), param1Edit->value(),
                   param2Edit->value(), minRadiusEdit->value(), maxRadiusEdit->value());
  qDebug() << "running";
  if (input.channels() == 1)
  ...
  for (size_t i = 0; i < circles.size(); i++) {
    ...
    QString info = QString("Center(%1, %2) Radius %3").arg(center.x).arg(center.y).arg(radius);
...
void HoughCircles::initParamsWidget() {
  ...
  param2Edit = createSpinBox(75, 0, 255);
...
bool FitEllipse::applyTo(const cv::Mat &input, cv::Mat &output) {
  ...
  int find_contours_mode = cv::CHAIN_APPROX_TC89_L1;        // CV_RETR_LIST
  cv::findContours(input, contours, find_contours_mode, cv::CHAIN_APPROX_NONE);
  std::sort(contours.begin(), contours.end(),
            [](std::vector<cv::Point> a, std::vector<cv::Point> b) { return b.size() < a.size(); });
  ...
  for (size_t i = 0, foundCount = 0; (i < contours.size()) && (foundCount < maxEllipseCount); i++) {
    ...
    cv::ellipse(output, box, cv::Scalar(pixVal, 0, 0), 1, cv::LINE_AA);
    cv::ellipse(output, box.center, box.size * 0.5f, box.angle, 0, 360, cv::Scalar(pixVal, pixVal, 0), 1, cv::LINE_AA);
